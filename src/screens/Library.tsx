import { BookIcon, SpeakerIcon } from "../components/Icons";
...
export function LibraryScreen({ onStart, onBack, onWarmUp }: { onStart: (s: Story, mode: Mode, resume: boolean) => void; onBack: () => void; onWarmUp: (thenSlug: string) => void }) {
  ...
  const pick = (s: Story) => {
    if (review.length > 0 && kid.role !== "listener") return onWarmUp(s.slug);   // words are due: run the warm-up, then open the book they tapped — never swallow the tap
      ...
      {confirm && inFlight && (
        <div className="pc confirmbox">
          ...
          <div className="btns">
            ...
            <button className="yes" onClick={() => { setSession(null); setConfirm(null); setPicked(confirm); }}>Start {confirm.title}</button>
          </div>
        </div>
      )}
      {groups.map((g) => {
